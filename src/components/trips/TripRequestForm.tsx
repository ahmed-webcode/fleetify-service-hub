...
import { CreateTripRequestDto } from "@/types/trip";
...
const formSchema = z
    .object({
        ...
        passengerCount: z.coerce.number().int().min(1, "Passenger count must be at least 1"),
    })
...
export function TripRequestForm({ onSuccess, onCancel }: TripRequestFormProps) {
    ...
    const onSubmit = async (data: TripFormValues) => {
        try {
            ...
            toast.error("Failed to submit trip request: " + (error.message || "Unknown error"));
        } finally {
            setLoading(false);
        }
    };
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            <FormControl>
                                <Input
                                    placeholder="e.g., Client's Address, Project Site"
                                    {...field}
                                />
                            </FormControl>
                            <FormMessage />
